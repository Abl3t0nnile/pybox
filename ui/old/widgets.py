...
class InventoryTab(Pile):
    ...
    _initContent = [
        ...
        _blank,
        Text(u"Zahnstocher"),
        Text(u"seltsame Münze"),
        Text(u"Brief"),
        Text(u"gefaltete Notiz"),
        Text(u"Messer"),
        Text(u"Holzteil"),
    ]
...
class StoryBoard(LineBox):
    ...
    def __init__(self) -> None:
        super(StoryBoard, self).__init__(self._story)
...
class SideBar(LineBox):
    ...
    def update_location(self, mode: str, obj: object) -> None:
        if mode == 'add':
            self._tabs.locTab.content.append(Text((u"{}".format(str(obj)))))
        elif mode == 'del':
            pass
        else:
            pass
